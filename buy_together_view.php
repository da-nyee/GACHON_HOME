...
?>
						<div class="form-group">		
							<span id="upfile">
								<a>첨부파일: </a>
								<a href="buy_together_download.php?num=<?=$num?>&real_name=<?=$real_name?>&file_name=<?=$file_name?>&file_type=<?=$file_type?>">
								<?=$file_name?></a>
								<a>(<?=$file_size?> byte)</a>
							</span>
						</div>
...
<?php 
	if($userid == $id){	    					
?>
      							<button type="button" class="btn btn-primary" onclick="location.href='./buy_together_modify_form.php?num=<?=$num?>&page=<?=$page?>'">수정하기</button>
	    						<a href="./buy_together_delete.php?num=<?=$num?>&page=<?=$page?>" class="btn btn-danger btn-xs" onclick="return confirmDelete()">삭제하기</a>
								<br/><br/>
<?php 
	}
?>
