		...
		<div class="container">
		  	<div class="card border-0 shadow my-5">
		    	<div class="card-body p-5">
		      		<form class="form-horizontal" id="notice_board_view">
...
?>
						<div class="form-group">		
							<span id="upfile">
								<a>첨부파일: </a>
								<a href="notice_board_download.php?num=<?=$num?>&real_name=<?=$real_name?>&file_name=<?=$file_name?>&file_type=<?=$file_type?>">
								<?=$file_name?></a>
								<a>(<?=$file_size?> byte)</a>
							</span>
						</div>
...
<?php 
	if($userid == $id){	    					
?>
      							<button type="button" class="btn btn-primary" onclick="location.href='./notice_board_modify_form.php?num=<?=$num?>&page=<?=$page?>'">수정하기</button>
	    						<a href="./notice_board_delete.php?num=<?=$num?>&page=<?=$page?>" class="btn btn-danger btn-xs" onclick="return confirmDelete()">삭제하기</a>
								<br/><br/>
<?php 
	}
?>
	    						<button type="button" class="btn btn-primary" onclick="location.href='./notice_board_list.php?page=<?=$page?>'">목록보기</button>
	    					</div>
	  					</div>
